...
function print_header {
   ...
   printf "%4s %5s %5s %3s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %-7s" "Tmax" "Tmean" "ERRc" "CPU" "FAN1" "FAN2" "FAN3" "FAN4" "FAN5" "FAN6" "FANA" "FANB" "Fan%0" "Fan%1" "MODE" 
}
...
#   DUTY1=$($IPMITOOL raw 0x30 0x70 0x66 0 1) # in hex
   ...
   RPM_FAN5=$(echo "$SDR" | grep "FAN5" | grep -Eo '[0-9]{3,5}')
   RPM_FAN6=$(echo "$SDR" | grep "FAN6" | grep -Eo '[0-9]{3,5}')
...
while [ 1 ] ; do
   ...
   # is no such zone.
	printf "%6.2f %3d %5s %5s %5s %5s %5s %5s %5s %5d %5d %5d %-7s" $ERRc $CPU_TEMP "${RPM_FAN1:----}" "${RPM_FAN2:----}" "${RPM_FAN3:----}" "${RPM_FAN4:----}" "${RPM_FAN5:----}" "${RPM_FAN6:----}" "${RPM_FANA:----}" "${RPM_FANB:----}" $DUTY0 $DUTY1 $MODEt
