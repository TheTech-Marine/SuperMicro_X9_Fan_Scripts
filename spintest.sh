...
function print_header {
   ...
   printf "%44s %29s \n" $SPACES "___Duty%___" "Curr_RPM___________________________________" 

   printf "%28s %9s %5s %5s %5s %5s %5s %5s %5s %5s %5s \n" "MODE" "Zone0" "Zone1" "FANA" "FANB" "FAN1" "FAN2" "FAN3" "FAN4" "FAN5" "FAN6"
}
...
##############################################
function print_line {
   printf "%-23s %-8s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s \n"  "$COND" $MODEt "${DUTY_0:----}" "${DUTY_1:----}" "${FANA:----}" "${FANB:----}" "${FAN1:----}" "${FAN2:----}" "${FAN3:----}" "${FAN4:----}" "${FAN5:----}" "${FAN6:----}"
...
#   echo -n `$IPMITOOL raw 0x30 0x70 0x66 1 0 $i`
   SPEED=$(echo "scale=0; ($i * 255) / 100" | bc)
